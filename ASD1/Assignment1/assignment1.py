...
"""
eda fucntion for subseting the data, after looking at the data's stats
"""


def eda_df(property_sales_df):
    """ Looking at the dataset and cleaning the dataset"""
...
def sales_property_type(property_sales_subset_df):
    """A line plot for various property- residential type sales over time"""
    ...
    for i in range(0, len(avg_sold_resident_df.columns)):
...
residential type and plotin in a stacked plot
"""


def total_sales(property_sales_subset_df):
    """A bar plot showing the total sales of residential type for every town"""
...
def market_cap(property_sales_subset_df):
    """A pie chart to captures the market of each property type"""
    ...
    patches, tests = ax_first.pie(
        first_five_property_count_df["Count"], startangle=90,
    ...
    patches, tests = ax_last.pie(
        last_five_property_count_df["Count"], startangle=90)
    # adding the legend to the chart
